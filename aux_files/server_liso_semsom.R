...
#removing inputplayers
    ...
    # )
      ...
      insertUI(selector = '#placeholder', where = "beforeEnd", multiple = F, 
                ui = fluidRow(
                 ...
                 inline(actionButton("mexer_dados", label = "Mexer os dados!"))
                 )
  ...
  observeEvent(input$readytoplay, priority = 1,{
  ...
  observeEvent(input$jogar_dados, priority = 1,{
    ...
    #jogou os dados, some o jogar e aparece mexer, mexeu os dados, o jogar aparece
    hideElement("jogar_dados")
    showElement("mexer_dados")
    showElement("linhas")
    print(length(escolhas))
    #auxiliar
    n_jogada_dado <<- n_jogada_dado + 1
    
    #para fazer ser de mão!!!!
  ...
  observeEvent(input$mexer_dados, priority = 2,{
    ...
    #colocando o icone de redo no lugar certo

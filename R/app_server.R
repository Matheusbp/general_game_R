...
folha_generator <- function(df) {
  ...
  observeEvent(input$readytoplay, priority = 2, {
    ...
    insertUI(
      selector = "#placeholder", where = "beforeEnd", multiple = T,
      ui = radioButtons("jogando",
        label = h4("Player:"),
    ...
    insertUI(
      selector = "#placeholder", where = "beforeEnd", multiple = F,
      ui = fluidRow(
        useShinyjs(),
  ...
  observeEvent(input$jogar_dados, priority = 2, {
    if (n_jogada_dado == 0) {
      ...
      if (aux_inicio_server == 0) {
        insertUI(
          ...
          ui = fluidRow(
            column(
              9,
              checkboxGroupInput("ficamdados",
                label = "Select the dices to hold",
      ...
      if (aux_inicio_server != 0) {
        updateCheckboxGroupInput(session, "ficamdados",
          label = "Select the dices to hold",
          choiceNames = lista_choicenames,
          choiceValues = 1:5,
    ...
    if (n_jogada_dado >= 1) {
      ...
      updateCheckboxGroupInput(session, "ficamdados",
        label = "Selecione os que ficarão",
        choiceNames = lista_choicenames,
        choiceValues = 1:5,
  ...
  observeEvent(input$jogar_dados, priority = 1, {
    if (aux_inicio_server == 0) {
      ...
      insertUI(
        selector = "#daquiprabaixotabela", where = "afterEnd", multiple = T,
        ui = fluidRow(
          column(
            5,
            inline(radioButtons("linhas",
              label = "Which line to add the values?",
              choices = nome_linhas,
              selected = NULL
            ))
          ),
          column(
            1,
            inline(textInput("valor",
              label = "Value to add",
              value = ""
            )),
            inline(actionButton("adicionar_valor", label = "Add to the table", width = "150px")),
            inline(actionButton("fim_jogada", label = "Finish the turn", width = "150px")),
            HTML(paste("<br></br>")),
            HTML(paste("<br></br>")),
            actionButton("revanche", label = "Restart the match!", width = "150px") # ,
          )
        )
      )
    }
